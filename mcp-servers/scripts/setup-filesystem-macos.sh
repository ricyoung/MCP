#!/bin/bash
# MCP Grid Setup Script - Filesystem (macOS)
...
# Create filesystem config for macOS
cat > mcp-servers/configs/filesystem-config-macos.json << 'EOF'
{
  "mcpServers": {
    "filesystem": {
      "command": "npx",
...
CONFIG_SOURCE="mcp-servers/configs/filesystem-config-macos.json"
...
if cp "$CONFIG_SOURCE" "$CLAUDE_CONFIG_PATH"; then
    ...
    echo "   Manual setup required - see filesystem-config-macos.json"
    exit 1
fi
...
echo "• /Users (User profiles and documents)"
