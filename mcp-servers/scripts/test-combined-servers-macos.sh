#!/bin/bash
# MCP Grid - Combined Server Testing Script (macOS)
...
for CONFIG_PATH in "${CLAUDE_CONFIG_PATHS[@]}"; do
    if [ -f "$CONFIG_PATH" ]; then
        ...
        if jq -e '.mcpServers."sequential-thinking"' "$CONFIG_PATH" &> /dev/null; then
            echo "   ✅ Sequential Thinking: CONFIGURED"
        else
            echo "   ❌ Sequential Thinking: NOT CONFIGURED"
        fi
        
        if jq -e '.mcpServers.filesystem' "$CONFIG_PATH" &> /dev/null; then
